...
class FF_MLP(nn.Module): #, FF_Model_Skeleton):
    def __init__(self):
        ...
        self.conv1 = FF_Conv2D(1, 32, 3, PUT REMAINING PARAMS)
        self.conv2 = FF_Conv2D(32, 64, 3, PUT REMAINING PARAMS)
        self.fc1 = FF_Linear(9216, 10, nn.functional.relu, PUT REMAINING PARAMS)
    ...
    def train_layers(self,
        ...
        for i in range(len(self.layers)):
            ...
            updated_dataset = FF_Dataset( (cur_layer(batch_pos), cur_layer(batch_neg)) for batch_pos, batch_neg in dataloader)
...
class FF_Linear(nn.Linear):
    ...
    def forward(self, 
        ...
        # Return f(W*X + b)
        return self.layer_activation(torch.mm(X, self.weight.T) + self.bias.unsqueeze(0)) # self.weight and self.bias part of nn.Linear
...
def main():
    ...
    FF_train_loader = torch.utils.dataset.DataLoader(train_data_labeled,batch_size=train_loader.batch_size, shuffle=True)
